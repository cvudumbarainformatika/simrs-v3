<template>
  <div class="app-table f-12">
    ...
    <div class="flex items-center justify-between q-py-sm print-hide" :class="useFull ? 'bg-primary round-edge' : ''">
      <!-- title -->
      ...
      <div class="col-one flex items-center">
        <div v-if="adaCari">
          <q-input v-model="search" outlined class="search-big" borderless :debounce="debounce" clearable dense
            :dark="useFull ? true : false" :placeholder="textCari" @keydown.enter.prevent="searchEnter">
            <template #prepend>
              <q-icon name="icon-mat-search" size="20px" />
            </template>
          </q-input>
        </div>
        <slot name="header-left-after-search" class="q-ml-md" />
      </div>
      <div class="col-one">
        <div class="row no-wrap items-center">
          ...
          <!-- refresh Ids -->
          <q-btn v-if="adaRefresh" unelevated round :color="useFull ? 'yellow' : 'blue-grey'" size="sm"
            icon="icon-mat-refresh" @click="emits('refresh')">
            <q-tooltip class="primary" :offset="[10, 10]">
              Refresh Table
            </q-tooltip>
          </q-btn>
          ...
          <!-- data baru -->
          <q-btn v-if="addData && adaTambah" class="q-ml-sm" unelevated round :color="useFull ? 'green' : 'primary'"
            size="sm" icon="icon-mat-add" @click="emits('newData')">
            <q-tooltip class="primary" :offset="[10, 10]">
              Data Baru
            </q-tooltip>
          </q-btn>
          <!-- style -->
          <q-btn v-if="useFull" flat
            :icon="!style.componentfull ? 'icon-mat-open_in_full' : 'icon-mat-close_fullscreen'" round
            :color="style.componentfull ? 'green' : 'white'" size="12px" class="q-ml-md"
    ...
    <q-separator />
    <!-- table -->
    <q-markup-table ref="refCellTable" dense flat bordered :separator="separator" class="screenwide">
      <thead>
        <tr v-if="enableHead">
      ...
      <tbody v-if="!loading">
        ...
        <template v-for="(item, i) in items" v-else :key="i">
          ...
          <tr v-if="item.expand">
            <td
              :colspan="isChecked && rowNo && !defaultBtn ? filterColumn?.length + 3 : ((isChecked || rowNo || !defaultBtn) ? filterColumn?.length + 2 : filterColumn?.length)">
    ...
    <!-- Pagination -->
    <AppPaginationTable v-if="items?.length > 0 && adaPaginasi && !simplePaginasi" class="print-hide" :meta="meta"
      :bg="pgnBg" :btn-color="pgnBtn" :text-color="pgnText" @first="emits('goto', 1)"
      @last="emits('goto', meta.last_page)" @next="emits('goto', meta.current_page + 1)"
      @prev="emits('goto', meta.current_page - 1)" />
    <!-- simle paginate -->
    <div v-if="simplePaginasi" class="row items-center justify-between q-pa-sm" :class="`${color} text-${textColor}`">
      <div>
        <div class="row items-center">
          <q-btn flat :color="textColor" icon="icon-mat-skip_previous" size="sm" round
            :disable="meta.current_page === 1" @click="emits('goto', 1)" />
          <q-btn flat :color="textColor" icon="icon-mat-chevron_left" size="sm" round :disable="meta.current_page === 1"
            @click="emits('goto', meta.current_page - 1)" />
          <div class="q-px-sm">
            <div v-if="meta.total !== 0" classs="row items-center">
              | <span class="q-px-sm">Halaman <span class="f-18 text-orange text-weight-bold">{{ meta.current_page }}
                </span> <span v-if="meta.from"> data ke - {{ meta.from }} sampai data ke - </span> <span v-if="meta.to"
                  class="text-weight-bold">{{ meta.to }}</span> </span> |
            </div>
            <div v-else>
              Tidak Ada Data
            </div>
          </div>
          <q-btn flat :color="textColor" icon="icon-mat-chevron_right" size="sm" round :disable="!meta.next_page_url"
            @click="emits('goto', meta.current_page + 1)" />
          <!-- <q-btn
            flat
            :color="textColor"
