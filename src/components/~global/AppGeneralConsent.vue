<template>
  <q-dialog persistent :maximized="maximizedToggle" transition-show="slide-left" transition-hide="slide-right"
    ...
    <q-card flat>
      ...
      <q-card-section>
        ...
        <div class="row justify-between q-mt-md q-col-gutter-md bg-grey-3">
          ...
          <div class="col-6 q-pa-md">
            <q-card class="q-pa-md">
              <div>Memberikan kuasa/ wewenang terhadap informasi kesehatan dan pengambilan keputusan kepada pihak yang
                di tunjuk pasien</div>
              <q-separator />
              <q-card-section>
                <div class="row">
                  <div class="col-auto">
                    <q-avatar color="primary" text-color="white" size="25px" class="q-mr-md">
                      1
                    </q-avatar>
                  </div>
                  <div class="col full-width">
                    <div class="full-width">
                      <app-input v-model="store.form.wali1" label="Yang diberi Kuasa" class="q-mb-xs" outlined valid />
                    </div>
                    <div class="full-width q-gutter-xs q-mt-sm">
                      <div>Hubungan dengan Pasien ?</div>
                      <q-radio v-for="item in store.wali" :key="item" v-model="store.form.hubunganWali1" :val="item"
                        :label="item" color="teal" dense size="xs" />

                      <q-btn flat round color="red" icon="icon-mat-refresh" dense
                ...
                <div class="row">
                  ...
                  <div class="col full-width">
                    <div class="full-width">
                      <app-input v-model="store.form.wali2" label="Yang diberi Kuasa" class="q-mb-xs" outlined valid />
                    </div>
                    <div class="full-width q-gutter-xs q-mt-sm">
                      <div>Hubungan dengan Pasien ?</div>
                      <q-radio v-for="item in store.wali" :key="item" v-model="store.form.hubunganWali2" :val="item"
                        :label="item" color="teal" dense size="xs" />
      ...
      <q-card-section>
        <div class="text-center ">
          ...
          <q-btn color="teal" class="q-my-md" @click="emits('openPreviewGc')">
            <q-icon left size="sm" name="icon-mat-menu_book" />
            <div>Informasi General Consent</div>
          </q-btn>
        </div>
        <div class="column flex-center q-mt-lg">
          <q-checkbox v-model="pernyataan" label="Saya
