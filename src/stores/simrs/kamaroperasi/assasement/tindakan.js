...
export const useTindakanKamarOperasiStore = defineStore('tindakan-ranap-store', {
  state: () => ({
  ...
  actions: {
    ...
    async getTindakan (pasien) {
      try {
        const resp = await api.get('v1/simrs/penunjang/ok/tindakan/listtindakan', { params: { noreg: pasien?.noreg, kodepoli: pasien?.kodepoli } })
    ...
    async saveTindakan (pasien) {
      ...
      return new Promise((resolve, reject) => {
        api.post('v1/simrs/penunjang/ok/tindakan/simpan', form)
          .then((resp) => {
            if (resp.status === 200) {
    ...
    async getNota (pasien) {
      ...
      const resp = await api.get('v1/simrs/penunjang/ok/tindakan/nota', params)
...
if (import.meta.hot) {
  import.meta.hot.accept(acceptHMRUpdate(useTindakanKamarOperasiStore, import.meta.hot))
}
