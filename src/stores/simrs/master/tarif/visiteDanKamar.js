...
export const useMasterVisiteDanKamarStore = defineStore('master_visite_dan_kamar', {
  state: () => ({
    isOpen: false,
    edit: false,
    loading: false,
  ...
  actions: {
    ...
    editData (payload) {
      ...
      key.forEach(a => {
        ...
        if (a == 'ruangan') {
          ...
          ruangans.forEach(b => {
            const found = this.ruangRanap?.find(c => c.kode == b)
            if (found) {
    ...
    async getDataTable () {
      ...
      try {
        ...
        const resp = await api.get('v1/simrs/master/tarif/visite-dan-kamar/list', param)
    ...
    async saveForm () {
      ...
      this.loading = true
      await api.post('v1/simrs/master/tarif/visite-dan-kamar/simpan', this.form)
        .then(resp => {
          this.loading = false
    ...
    async deleteData (val) {
      this.loading = true
      await api.post('v1/simrs/master/tarif/visite-dan-kamar/hapus', val)
        .then(resp => {
          this.loading = false
    ...
    async undeleteData (val) {
      this.loading = true
      await api.post('v1/simrs/master/tarif/visite-dan-kamar/tampilkan', val)
        .then(resp => {
          this.loading = false
...
if (import.meta.hot) {
  import.meta.hot.accept(acceptHMRUpdate(useMasterVisiteDanKamarStore, import.meta.hot))
}
