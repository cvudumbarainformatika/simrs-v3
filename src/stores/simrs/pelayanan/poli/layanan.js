...
export const useLayananPoli = defineStore('layanan-poli', {
  state: () => ({
    ...
    tabs: ['Diagnosa Medik', 'Tindakan Medik', 'Prosedur (Icd 9)', 'Rencana Pengobatan', 'Diagnosa Keperawatan', 'Diagnosa Kebidanan', 'Laporan Tindakan', 'Laporan ESWL', 'Pra Anestesia', 'Pra Bedah',], // diagnosa di hapus
    // diagnosa
    searchdiagnosa: '',
    listDiagnosa: [],
    listTindakan: [],
    ...
    async saveTindakan (pasien) {
      ...
      try {
        ...
        if (resp.status === 200) {
          ...
          kamarOP.injectDataPasien(pasien, isi, 'tindakan')
    ...
    async hapusTindakan (pasien, id) {
      ...
      try {
        ...
        if (resp.status === 200) {
          ...
          kamarOP.hapusDataTindakan(pasien, id)
          this.setNotas(resp?.data?.nota)
          notifSuccess(resp)
        }
      }
