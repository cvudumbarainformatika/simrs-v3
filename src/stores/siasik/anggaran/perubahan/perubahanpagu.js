...
export const usePenetapanPerubahanPaguStore = defineStore('penetapan-perubahan-pagu-store', {
  state: () => ({
    items: [],
    loading: false,
    loadingSave: false,
  ...
  actions: {
    ...
    async simpanData() {
      ...
      try {
        const resp = await api.post('v1/anggaran/perubahan/pagu/save', this.form)
    ...
    async getData() {
      this.loading = true
      const params = { params: this.params }
      const resp = await api.get('/v1/anggaran/perubahan/pagu/index', params)
    ...
    async deleteData(id) {
      ...
      const payload = { id }
      try {
        const resp = await api.post('/v1/anggaran/perubahan/pagu/delete', payload)
        if (resp.status === 200) {
          // this.items = resp?.data?.data
          notifSuccess(resp)
          this.getData()
    ...
    async kunciData(id) {
      ...
      const payload = { id }
      try {
        const resp = await api.post('/v1/anggaran/perubahan/pagu/kunci', payload)
        if (resp.status === 200) {
          // this.items = resp?.data?.data
          notifSuccess(resp)
          this.getData()
