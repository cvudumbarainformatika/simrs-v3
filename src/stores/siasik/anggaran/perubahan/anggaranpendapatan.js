...
export const usePerubahanAnggaranPendapatanStore = defineStore('perubahan-anggaran-pendapatan-store', {
  state: () => ({
    items: [],
    loading: false,
    loadingSave: false,
  ...
  actions: {
    ...
    async simpanData() {
      ...
      try {
        const resp = await api.post('v1/anggaran/perubahan/pendapatan/save', this.form)
    ...
    async penetapan() {
      this.loadingSave = true
      try {
        const resp = await api.post('v1/anggaran/perubahan/pendapatan/penetapan', this.params)
    ...
    async getData() {
      this.loading = true
      const params = { params: this.params }
      const resp = await api.get('/v1/anggaran/perubahan/pendapatan/index', params)
    ...
    async deleteData(id) {
      ...
      const payload = { id }
      try {
        const resp = await api.post('/v1/anggaran/perubahan/pendapatan/delete', payload)
        if (resp.status === 200) {
          // this.items = resp?.data?.data
          notifSuccess(resp)
          this.getData()
    ...
    async kunciData(id) {
      ...
      const payload = { id }
      try {
        const resp = await api.post('/v1/anggaran/perubahan/pendapatan/kunci', payload)
        if (resp.status === 200) {
          // this.items = resp?.data?.data
          notifSuccess(resp)
          this.getData()
