...
export const usePergeseranPaguStore = defineStore('pergeseran-pagu-store', {
  state: () => ({
    items: [],
    loading: false,
    loadingSave: false,
  ...
  actions: {
    ...
    async simpanData() {
      ...
      try {
        const resp = await api.post('v1/anggaran/pergeseran/pagu/save', this.form)
    ...
    async getData() {
      this.loading = true
      const params = { params: this.params }
      const resp = await api.get('/v1/anggaran/pergeseran/pagu/index', params)
