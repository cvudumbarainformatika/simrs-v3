...
export const usePergeseranPendapatanStore = defineStore('pergeseran-pendapatan-store', {
  state: () => ({
    items: [],
    loading: false,
    loadingSave: false,
  ...
  actions: {
    ...
    getRekening() {
      ...
      return new Promise((resolve) => {
        api.get('v1/anggaran/pergeseran/pendapatan/getrekening', params).then((resp) => {
    ...
    async simpanData() {
      ...
      try {
        const resp = await api.post('v1/anggaran/pergeseran/pendapatan/save', this.form)
    ...
    async getData() {
      this.loading = true
      const params = { params: this.params }
      const resp = await api.get('/v1/anggaran/pergeseran/pendapatan/index', params)
    ...
    editForm(val) {
      ...
      this.form.nilai = val?.nilai_pergeseran
    ...
    async deleteData(id) {
      ...
      const payload = { id }
      try {
        const resp = await api.post('/v1/anggaran/pergeseran/pendapatan/delete', payload)
        if (resp.status === 200) {
          // this.items = resp?.data?.data
          notifSuccess(resp)
          this.getData()
    ...
    async kunciData(id) {
      ...
      const payload = { id }
      try {
        const resp = await api.post('/v1/anggaran/pergeseran/pendapatan/kunci', payload)
        if (resp.status === 200) {
          // this.items = resp?.data?.data
          notifSuccess(resp)
          this.getData()
