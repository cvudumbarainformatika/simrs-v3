...
export const usePergeseranPerubahanPaguStore = defineStore('pergeseran-perubahan-pagu-store', {
  state: () => ({
    items: [],
    loading: false,
    loadingSave: false,
  ...
  actions: {
    ...
    async simpanData() {
      ...
      try {
        const resp = await api.post('v1/anggaran/pergeseranperubahan/pagu/save', this.form)
    ...
    async getData() {
      this.loading = true
      const params = { params: this.params }
      const resp = await api.get('/v1/anggaran/pergeseranperubahan/pagu/index', params)
    ...
    editForm(val) {
      ...
      this.form.total = val?.pagu_pergeseran
