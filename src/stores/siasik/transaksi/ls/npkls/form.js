...
export const useFormNPKlangsungStore = defineStore('formNPKlangsung', {
  state: () => ({
    loading: false,
    loadingHapus: false,
    loadingrinci: false,
  ...
  actions: {
    initReset(data) {
      ...
      if (data) {
        return new Promise((resolve) => {
          ...
          this.formheader.nonpk = data?.nonpk
    ...
    async saveData(add) {
      ...
      return new Promise((resolve, reject) => {
        api.post('/v1/transaksi/npkls/savedata', this.formheader)
          .then((resp) => {
            console.log('Resp Api NOTA DINAS', resp?.data)

            this.formheader.nonpk = resp.data?.result?.nonpk
            this.rinci.nonpk = resp.data?.result?.nonpk
            this.paramsrinci.nonpk = resp.data?.result?.nonpk
    ...
    async loadData(nonpk) {
      this.loading = true;
      try {
        const response = await api.get(`/v1/transaksi/npkls/${nonpk}`);
    ...
    hapusRinci(row) {
      ...
      return new Promise(resolve => {
        api.post('/v1/transaksi/npkls/deleterinci', row)
          .then(resp => {
    ...
    getDataNpdls() {
      ...
      const params = { params: this.params }
      return new Promise((resolve) => {
        api.get('/v1/transaksi/npkls/selectnpd', params)
          .then((resp) => {
            if (resp.status === 200) {
    ...
    async listrincians() {
      ...
      return new Promise((resolve, reject) => {
        api.get('/v1/transaksi/npkls/getrincian', params).then((resp) => {
          if (resp.status === 200) {
