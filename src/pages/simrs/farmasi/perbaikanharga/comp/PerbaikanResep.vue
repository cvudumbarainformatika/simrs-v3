...
function onClickRow (item) {
  ...
  if (allBatches.length === 1) {
    ...
    $q.dialog({
      ...
      emits('ubahharga', { item, penerimaan: singleBatch, harga: singleBatch.harga, tipe: 'resep', id: item.id })
    })
  } else {
    ...
    $q.dialog({
      ...
      emits('ubahharga', { item, penerimaan: selectedBatch, harga: selectedBatch.harga, tipe: 'resep', id: item.id })
    })
  }
}
