      ...
      <div v-for="(item, i) in props.data?.data?.racikan" :key="item">
        ...
        <div v-else>
          <div class="row items-center"
            ...
            <div class="col-2">
              {{ item?.nobatch ?? cekHarga(item)?.nobatch }}
            </div>
          </div>
        </div>
      </div>
...
function onClickRow (item) {
  ...
  if (allBatches.length === 1) {
    ...
    $q.dialog({
      ...
      emits('ubahharga', { item, penerimaan: singleBatch, harga: singleBatch.harga, tipe: 'racikan', id: item.id })
    })
  } else {
    ...
    $q.dialog({
      ...
      emits('ubahharga', { item, penerimaan: selectedBatch, harga: selectedBatch.harga, tipe: 'racikan', id: item.id })
    })
  }
}

function cekHarga (item) {
  if (item?.nobatch == null) {
