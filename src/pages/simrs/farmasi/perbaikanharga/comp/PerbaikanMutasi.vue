...
function onClickRow (item) {
  ...
  if (allBatches.length === 1) {
    ...
    $q.dialog({
      ...
      emits('ubahharga', { item, penerimaan: singleBatch, harga: singleBatch.harga, tipe: 'mutasi', id: item.id })
    })
  } else {
    ...
    $q.dialog({
      ...
      emits('ubahharga', { item, penerimaan: selectedBatch, harga: selectedBatch.harga, tipe: 'mutasi', id: item.id })
    })
  }
}
