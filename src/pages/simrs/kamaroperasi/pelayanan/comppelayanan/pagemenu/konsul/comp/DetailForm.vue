...
async function updateFlagRanap(item) {
  const findPasien = storeOK.items?.find(x => x?.noreg === props?.pasien?.noreg) ?? null
...
function onSubmit() {
  ...
  form.value.kdgroup_ruangan = 'PEN001'
  form.value.kdruang = 'PEN001'
  ...
  return new Promise((resolve, reject) => {
    api.post('v1/simrs/ranap/layanan/konsultasi/updateJawaban', form.value)
      .then(resp => {
        ...
        if (resp.status === 200) {
          storeOK.injectDataPasien(pasien, resp.data.result, 'konsultasi')
