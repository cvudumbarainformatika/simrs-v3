<template>
  <div style="max-width: 100%">
    ...
    <q-list bordered class="q-pa-sm">
      <div class="column">
        <q-item class="q-my-sm full-width" clickable v-ripple v-for="(item, n) in props.pasien?.suratketerangandokter"
          :key="n">
          <q-item-section>
            <q-item-label class="text-subtitle1">{{ item?.nosurat }}</q-item-label>
            <q-item-label caption lines="2">
              Keperluan: {{ item?.untukKeperluan }} ||
              <b> Kesimpulan: {{ item?.kesimpulan }}</b>
            </q-item-label>
          </q-item-section>
