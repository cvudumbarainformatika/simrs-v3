...
<template>
  <div class="row justify-end text-weight-bold q-gutter-sm" style="margin-right: 20px;margin-top: 10px;">
    <div class="col-1">
      <q-select v-model="store.params.tahunawal" label="Tahun" :options="options" outlined option-label="nama"
        emit-value map-options="" />
    </div>
    <div class="col-1">
      <q-select v-model="store.params.tahunakhir" label="Tahun" outlined :options="options" option-label="nama"
        emit-value map-options="" />
    </div>
    <div class="col-1">
      <q-btn unelevated color="dark" round size="sm" icon="icon-mat-search" @click="carikunjungan">
          ...
          <tr v-for="(lasoapb, s) in store?.items" :key="s">
            ...
            <td class="text-left f-12 " width="16%">
              <div v-for="(anamnese, anam) in lasoapb?.anamnesis" :key="anam">
                ...
                <div v-if="anamnese?.riwayatpenyakitsekarang !== ''">
            ...
            <td class="text-left f-12" width="25%">
              <div v-for="(pemeriksaanfisik, fisik) in lasoapb?.pemeriksaanfisik" :key="fisik">
                <div>*) Nadi: {{ pemeriksaanfisik?.rs4 }}, Pernapasan: {{ pemeriksaanfisik?.pernapasan }}, Sistole: {{
                  pemeriksaanfisik?.sistole }}, Diastole: {{ pemeriksaanfisik?.diastole }}</div>
                <div>*) Suhu Tubuh: {{ pemeriksaanfisik?.suhutubuh }}, Tinggi Badan: {{ pemeriksaanfisik?.tinggibadan
                  }}, Berat badan: {{ pemeriksaanfisik?.beratbadan }},Vas: {{ pemeriksaanfisik?.vas }}</div>
                <div>*) Status Psikologi: {{ pemeriksaanfisik?.statuspsikologis }}, Sosial Ekonomi: {{
                  pemeriksaanfisik?.sosialekonomi }}</div>
            ...
            <td class="text-left f-12" width="20%">
              <div v-for="(diagnosa, diagx) in lasoapb?.diagnosa" :key="diagx">
                <div>*) {{ diagnosa?.masterdiagnosa?.rs1 }} {{ diagnosa?.masterdiagnosa?.rs4 }}</div>
              </div>
              <q-separator v-if="lasoapb?.diagnosakeperawatan?.length" color="dark" />
              <br>
              <div v-for="(diagnosakeperawatan, kep) in lasoapb?.diagnosakeperawatan" :key="kep">
            ...
            <td class="text-left f-12" width="25%">
              <div v-if="lasoapb?.laborat?.length">
                <u><b> Laborat </b></u>
                <div v-for="(laborat, lab) in lasoapb?.laborat" :key="lab">
                  <div>*) {{ laborat?.pemeriksaanlab?.rs2 }} : {{ laborat?.rs21 }}</div>
                </div>
              </div>
              <div v-if="lasoapb?.pembacaanradiologi?.length">
                <u><b> Radiologi </b></u>
                <div v-for="(pembacaanradiologi, w) in lasoapb?.pembacaanradiologi" :key="w">
                  <div>*) {{ pembacaanradiologi?.rs3 ? pembacaanradiologi?.rs3 : '-' }}</div>
                </div>
              </div>
              <br>
              <div v-if="lasoapb?.apotekrajal?.length || lasoapb?.apotekrajalpolilalu?.length
                || lasoapb?.apotekracikanrajal?.length || lasoapb?.apotekracikanrajallalu?.length">
                <u><b> Obat </b></u>
                <div v-for="(obatlalu, o) in lasoapb?.apotekrajalpolilalu" :key="o">
                  <div>*) {{ obatlalu?.obat }}</div>
                </div>
                <div v-for="(obat, o) in lasoapb?.apotekrajal" :key="o">
                  <div>*) {{ obat?.obat }}</div>
                </div>
                <div v-for="(obatracikan, o) in lasoapb?.apotekracikanrajal" :key="o">
                  <div>*) {{ obatracikan?.obat }}</div>
                </div>
                <div v-for="(obatracikanlalu, o) in lasoapb?.apotekracikanrajallalu" :key="o">
              ...
              <div v-if="lasoapb?.tindakan?.length">
                <u><b> Tindakan </b></u>
                <div v-for="(tindakan, o) in lasoapb?.tindakan" :key="o">
                  <div>*) {{ tindakan?.tindakan }} {{ tindakan?.keterangan ?? '' }}</div>
                </div>
              </div>
              <div v-if="lasoapb?.kamaroperasi?.length">
                <u><b> Operasi </b></u>
                <div v-for="(kamaroperasi, ok) in lasoapb?.kamaroperasi" :key="ok">
              ...
              <div v-if="lasoapb?.usg?.length || lasoapb?.ecg?.length || lasoapb?.eeg">
                <u><b> ParaKlinik </b></u>
                <div v-for="(usg, ok) in lasoapb?.usg" :key="ok">
                  <div>*) {{ usg?.nama }} {{ usg?.hasil }}</div>
                </div>
                <br>
                <div v-for="(ecg, ec) in lasoapb?.ecg" :key="ec">
                  <div>*) {{ ecg?.mastertindakan?.rs2 }} {{ ecg?.hasil }}</div>
                </div>
                <br>
                <div v-for="(eeg, ec) in lasoapb?.eeg" :key="ec">
                  <div>*) {{ eeg?.klasifikasi }} {{ eeg?.impresi }}</div>
                </div>
              </div>
              <q-separator v-if="cariPlaningKep(lasoapb?.diagnosakeperawatan)?.length" color="dark" />
              <div v-if="cariPlaningKep(lasoapb?.diagnosakeperawatan)?.length">
                <u><b> Planing Keperawatan </b></u>
                <div v-for="plann in cariPlaningKep(lasoapb?.diagnosakeperawatan)" :key="plann">
...
function cariPlaningKep (val) {
  const data = val?.flatMap((item) => item?.intervensi)?.filter(x => x?.masterintervensi?.group === 'plann')
...
.warna-garis .q-table__container .q-table--bordered table {
  border: 1px solid black;
  /* warna garis luar */
}

.warna-garis .q-table__container .q-table--bordered table td,
.warna-garis .q-table__container .q-table--bordered table th {
  border: 1px solid black;
  /* warna garis luar */
}
