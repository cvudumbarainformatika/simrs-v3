<template>
  <div>
    <div v-if="!props?.lists?.length" class="column flex-center text-white" style="min-height:400px">
      No Data Available
    </div>
    <q-list v-else bordered class="rounded-borders bg-white" separator square>
      ...
      <q-item v-for="row in lists" :key="row?.id" v-ripple clickable class="q-py-xs"
        :class="activated === row.id ? 'bg-yellow-3' : ''" @click="activated = row.id">
        <q-item-section style="max-width: 100px;">
          {{ row.kode }}
        </q-item-section>
