<template>
  <q-dialog v-model="isOpen" persistent full-width max-width="980px">
    <q-card style="width: 950px; max-width: 95vw;" class="bg-white text-black font-sans">
      <!-- HEADER DIALOG (PURELY FOR VIEWING / PREVIEWING DETAIL) -->
      ...
      <q-card-section class="q-pa-md overflow-auto" style="max-height: 82vh;">
        <div class="status-sedasi-doc text-black">
            ...
            <table class="full-width table-grid-chart q-my-xs text-center">
              <thead>
                <tr class="bg-grey-2">
                  ...
                  <th v-for="(row, idx) in monitoringRows" :key="idx" width="6.5%">{{ row.waktu || '-' }}</th>
                  <th v-for="emptyIdx in (12 - monitoringRows.length)" :key="'e-'+emptyIdx" width="6.5%">-</th>
                </tr>
                <tr>
                  <td><strong>Obat-obatan & Rute Pemberian</strong></td>
                  <td v-for="(row, idx) in monitoringRows" :key="idx" class="font-9 font-bold">{{ row.obat_sedasi || '-' }}</td>
                  <td v-for="emptyIdx in (12 - monitoringRows.length)" :key="'eo-'+emptyIdx">-</td>
                </tr>
              </thead>
            </table>
            ...
            <table class="full-width table-grid-chart text-center q-mb-xs">
              <thead>
                <tr class="bg-grey-3">
                  ...
                  <th width="5%">Nadi</th>
                  <th width="5%">TD</th>
                  <th v-for="(row, idx) in monitoringRows" :key="idx" width="6.5%" class="font-9">{{ row.waktu }}</th>
                  <th v-for="emptyIdx in (12 - monitoringRows.length)" :key="'egh-'+emptyIdx" width="6.5%">-</th>
                </tr>
              </thead>
              <tbody>
                <tr v-for="(level, lIdx) in gridLevels" :key="lIdx">
                  ...
                  <td v-for="(row, rIdx) in monitoringRows" :key="rIdx" class="cell-plot font-bold">
                    {{ getGridCellContent(row, level) }}
                  </td>
                  <td v-for="emptyIdx in (12 - monitoringRows.length)" :key="'egc-'+emptyIdx"></td>
                </tr>
              </tbody>
            </table>

            <!-- GRAFIK PEMANTAUAN VITAL SIGNS (RAPI: SUMBU KIRI 0-240, SUMBU KANAN WAKTU UTAMA, SEJAJAR DENGAN MARKER X DAN O) -->
              ...
              <div class="row no-wrap items-stretch">
                ...
                <div class="col relative-position overflow-hidden" style="height: 165px;">
                  <svg class="full-width fit" viewBox="0 0 540 165" preserveAspectRatio="none">
                    ...
                    <g v-for="(point, pIdx) in dotPlotData" :key="pIdx">
          ...
          <q-separator class="q-my-md" color="grey-4" style="height: 2px;" />

          <!-- ==================== LEMBAR 2: PASCA SEDASI & DISCHARGE ==================== -->
          <div class="bg-white q-pa-sm">
...
const monitoringRows = computed(() => {
  return parseJSON(props.item?.monitoring_intra)
})
...
const dotPlotData = computed(() => {
  ...
  return rows.slice(0, 12).map((row, i) => {
    ...
    let suhuRaw = row.suhu || props.item?.suhu_pra || props.item?.assesmen_pra_sedasi_suhu
    let suhu = null
