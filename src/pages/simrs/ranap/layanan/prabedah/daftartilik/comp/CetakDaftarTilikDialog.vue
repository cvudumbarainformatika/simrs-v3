...
const bodyMarkerImg = computed(() => {
  const k = (props.pasien?.kelamin || props.item?.kelamin || '').toLowerCase()
...
const qrUrl = (kdpeg) => {
  if (!kdpeg) return ''
  const noreg = props.pasien?.noreg || props.item?.noreg || ''
...
const printObj = {
  id: 'print-daftartilik-dialog-doc',
...
<template>
  ...
  >
    <q-card class="bg-grey-3 fit column">
      <!-- Toolbar Header (Hilang saat cetak) -->
      <q-card-section class="bg-indigo-10 text-white row items-center justify-between q-py-xs no-print shadow-2">
        <div class="row items-center q-gutter-x-sm">
          <q-icon name="icon-mat-print" size="22px" />
          <span class="text-subtitle1 text-weight-bold">Pratinjau Cetak: Daftar Tilik Pre & Paska Operasi</span>
        </div>
        <div class="row items-center q-gutter-x-sm">
          <q-btn
            v-print="printObj"
      ...
      <q-card-section class="col scroll q-pa-md flex justify-center">
        <div id="print-daftartilik-dialog-doc" class="bg-white print-area q-pa-md shadow-3 text-black text-body2 font-body font-print border-print" style="width: 210mm; min-height: 297mm; box-sizing: border-box;">
          
          <!-- Kop Surat Standard RS -->
          <AppKopSuratStandard :pasien="pasien" :header="['DAFTAR TILIK', 'VERIFIKASI PASIEN OPERASI']" />
          ...
          <div class="text-center text-weight-bold text-subtitle2 uppercase tracking-wide q-my-xs text-black border-bottom-double q-pb-xs">
...
@media print {
  ...
  /* Sembunyikan seluruh komponen Quasar Layout & UI non-print secara global */
  .q-header,
  .q-footer,
  .q-drawer,
  .q-drawer-container,
