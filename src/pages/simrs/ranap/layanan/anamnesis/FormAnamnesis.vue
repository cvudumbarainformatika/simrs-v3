<template>
  <div class="row q-col-gutter-md">
    ...
    <q-input v-if="!ulang" v-model="store.form.rwPenySkr" outlined autogrow stack-label standout="bg-yellow-3"
      label="Riwayat Penyakit Sekarang" hide-bottom-space style="width:50%" />
    <q-input v-if="!ulang" v-model="store.form.rwPenyDhl" outlined autogrow stack-label standout="bg-yellow-3"
      label="Riwayat Penyakit Dahulu" hide-bottom-space style="width:50%" />
    <q-input v-if="!ulang" v-model="store.form.rwPenyKlrg" outlined autogrow stack-label standout="bg-yellow-3"
      label="Riwayat Penyakit Keluarga" hide-bottom-space style="width:50%" />
    <q-input v-if="!ulang" v-model="store.form.rwPengobatan" outlined autogrow stack-label standout="bg-yellow-3"
      label="Riwayat Pengobatan" hide-bottom-space style="width:50%" />
    <q-input v-if="!ulang" v-model="store.form.rwPkrjDgZatBahaya" outlined autogrow stack-label standout="bg-yellow-3"
    ...
    <div v-if="nakes === '2' || nakes === '3'" class="col-12">
      ...
      <q-card v-if="gruping === '4.2'" flat bordered class="q-mb-sm">
        <q-card-section class="q-py-xs q-px-md">
          <div class="q-mt-sm">
            ...
            <q-radio v-model="store.formKebidanan.keluhannyeri.kajianNyeri" v-for="dd in store.pilihanNyeris" :key="dd"
              :label="dd?.text" :val="dd.text" @update:model-value="(val) => {
        ...
        <q-card-section>
          <div v-if="store.formKebidanan.keluhannyeri.kajianNyeri === 'Wong Baker Face Scale'"
            class="row items-center q-col-gutter-md">
            <div class="col-4">
              <div class="flex flex-center">
                <div class="">
                  <q-icon size="lg" color="teal" :name="iconNyeriKebidanan" />
                </div>
                <div><em class="text-primary q-ml-sm">{{ store.formKebidanan.keluhannyeri.ket }}</em></div>
              </div>
            </div>
            <div class="col-8">
              <q-slider v-model="store.formKebidanan.keluhannyeri.skorNyeri" color="primary" thumb-color="primary"
                label-color="primary" label-text-color="yellow" markers :marker-labels="(val) => fnMarkerLabel"
                marker-labels-class="text-primary" label-always switch-label-side :min="0" :max="10"
                @update:model-value="(val) => store.hitungSkorNyeri('kebidanan')" style="width: 100%;" />
            </div>
          </div>
          <div v-else>
            ...
            <template v-for="(val, key) in store.formKebidanan.keluhannyeri?.form" :key="val">
              <div class="row q-col-gutter-md">
                ...
                <div class="col-9">
                  <div class="flex q-gutter-sm items-center">
                    <q-radio dense size="sm" v-model="store.formKebidanan.keluhannyeri.form[key]"
                      v-for="aa in store.formNyeris?.find(x => x.kode === key)?.values" :key="aa" :label="aa?.text"
                      :val="aa" @update:model-value="(val) => {
                        // store.form.ekspresiWajahKet = aa?.text
                        // console.log('aa',val);
        ...
        <q-card-section class="q-pa-none">
          <q-list bordered separator>
            <q-item v-for="(val, key) in store.formNeoNatal.keluhannyeri?.form" :key="val">
              ...
              <q-item-section>
                <div class="flex q-gutter-sm items-center">
                  <q-radio dense size="sm" v-model="store.formNeoNatal.keluhannyeri.form[key]"
                    v-for="aa in store.formNyeriNeonatals?.find(x => x.kode === key)?.values" :key="aa"
                    :label="aa?.text" :val="aa" @update:model-value="(val) => {
        ...
        <q-card flat bordered>
          ...
          <template v-for="(val, key) in store.formKebidanan.skreeninggizi?.form" :key="val">
            <q-card-section class="row">
              ...
              <div class="col-3 flex q-gutter-sm justify-end">
                <q-radio v-for="aa in store.formGiziObgyns?.find(x => x.kode === key)?.values" dense
                  v-model="store.formKebidanan.skreeninggizi.form[key]" :val="aa" :label="aa?.text"
                  @update:model-value="store.hitungSkorSgk" />
              </div>
            </q-card-section>
            <q-separator />
          </template>
          ...
          <template v-for="(val, key) in store.formNeoNatal.skreeninggizi?.form" :key="val">
            <q-card-section class="row">
              ...
              <div class="col-9 flex q-gutter-sm">
                <q-radio v-for="aa in store.formGiziNeonatals?.find(x => x.kode === key)?.values" dense
                  v-model="store.formNeoNatal.skreeninggizi.form[key]" :val="aa" :label="aa?.text"
                  @update:model-value="store.hitungSkorSgn" />
              </div>
            </q-card-section>
            <q-separator />
          </template>
            ...
            <q-list separator>
              <q-item v-for="(val, aa) in store.formPediatrik?.skreeninggizi?.form" :key="aa">
                <q-item-section class="q-gutter-sm">
                  <div v-for="(ll, i) in store.formGiziPediatrik?.find(x => x?.kode === aa)?.label" :key="i"
                    :class="i === 0 ? '' : 'text-italic'">
                ...
                <q-item-section style="max-width:22%" top>
                  <div class="flex q-gutter-sm">
                    <q-radio dense v-for="n in store.formGiziPediatrik?.find(x => x?.kode === aa)?.values" :key="n"
                      size="sm" v-model="store.formPediatrik.skreeninggizi.form[aa]" :label="n?.text" :val="n"
                      @update:model-value="store.hitungSkorSgp" />
                  </div>
                </q-item-section>
              </q-item>
            </q-list>
      ...
      <q-card v-if="gruping === '4.2' && !ulang" flat bordered class="q-mb-sm">
        <q-card-section class="row q-col-gutter-xs">
          ...
          <q-input v-model="store.formKebidanan.rwObsetri" outlined label="Riwayat Peny. Obsetric/Gynecology Sekarang"
            standout="bg-yellow-3" autogrow class="col-12" />
          <q-input v-model="store.formKebidanan.rwRawat" outlined label="Pernah dirawat ?" standout="bg-yellow-3"
            autogrow class="col-6" />
          <q-input v-model="store.formKebidanan.rwOperasi" outlined label="Pernah Operasi ?" standout="bg-yellow-3"
            autogrow class="col-6" />
          ...
          <div class="col-12 q-mt-sm">
            ...
            <div class="flex q-gutter-sm">
              <q-checkbox dense v-for="(g, x) in store.gynecologys" :key="x" v-model="store.formKebidanan.rwGynecology"
                :val="g" :label="g" color="primary" />
              <app-input-simrs v-if="store.formKebidanan.rwGynecology.includes('Lain-lain')"
                v-model="store.formKebidanan.rwGynecologyLain" label="Jelaskan" class="full-width" />
            </div>
            <q-separator class="q-my-sm" />
            ...
            <div class="row q-col-gutter-xs">
              ...
              <q-select v-model="store.formKebidanan.siklus" dense standout="bg-yellow-3 text-black" outlined
                label="Siklus" :options="store.sikluses" emit-value map-options input-class="ellipsis" fill-input
                hide-bottom-space class="col-3" />
              <app-input-simrs v-model="store.formKebidanan.lamaMens" label="Lama (hr)" class="col-2"
                :valid="{ number: true }" />
              ...
              <app-input-simrs v-model="store.formKebidanan.kawinKe" label="Perkawinan Ke" class="col-2"
                :valid="{ number: true }" />
              <app-input-simrs v-model="store.formKebidanan.nikahUmur" label="Nikah Umur" class="col-2"
                :valid="{ number: true }" />
              ...
              <q-select v-model="store.formKebidanan.imunisasi" dense standout="bg-yellow-3 text-black" outlined
                label="Imunisasi" :options="store.imunisasis" emit-value map-options input-class="ellipsis" fill-input
              ...
              <div class="full-width">
                <div class="flex q-gutter-sm items-center q-mb-sm q-mt-xs">
                  ...
                  <q-btn dense outline color="primary" icon="icon-mat-add" label="Tambah Riwayat Kehamilan"
                    class="q-px-sm" @click="store.openDialogFormRiwayat = true" />
                </div>
                <TableRiwayatKehamilan :items="store.riwayatKehamilans"
                  @delete="(id) => store.deleteRiwayatKehamilan(pasien, id)" />
              </div>
            </div>
            <div class="text-bold q-mt-sm">
              Pola Eliminasi
            ...
            <div class="row q-col-gutter-xs">
              <app-input-simrs v-model="store.formKebidanan.bab" label="BAB (x/hr)" class="col-2"
                :valid="{ number: true }" />
              <q-select v-model="store.formKebidanan.konsistensi" dense standout="bg-yellow-3 text-black" outlined
                label="Konsistensi" :options="store.konsistensis" emit-value map-options input-class="ellipsis"
                fill-input hide-bottom-space class="col-3" />
              <q-select v-model="store.formKebidanan.warna" dense standout="bg-yellow-3 text-black" outlined
                label="Warna" :options="store.warnas" emit-value map-options input-class="ellipsis" fill-input
                hide-bottom-space class="col-4" />

              <app-input-simrs v-model="store.formKebidanan.peristatikUsus" label="Peri. Usus x/mnt" class="col-3"
                :valid="{ number: true }" />
              <q-select v-model="store.formKebidanan.flatus" dense standout="bg-yellow-3 text-black" outlined
                label="Flatus" :options="store.yaTidaks" emit-value map-options input-class="ellipsis" fill-input
              ...
              <app-input-simrs v-model="store.formKebidanan.jmlBak" label="Jml. BAK ml/hr" class="col-3"
                :valid="{ number: true }" />
              <!-- <q-select
                ...
                class="col-4"
              /> -->
              <q-select v-model="store.formKebidanan.warnaUrine" dense standout="bg-yellow-3 text-black" outlined
                label="Warna Urine" :options="store.warnaUrine" hide-selected fill-input use-input input-debounce="0"
                hide-bottom-space hide-dropdown-icon @input-value="(val) => store.formKebidanan.warnaUrine = val"
                class="col-4" />
              <q-select v-model="store.formKebidanan.kateter" dense standout="bg-yellow-3 text-black" outlined
                label="Terpsg Kateter" :options="store.yaTidaks" emit-value map-options input-class="ellipsis"
                fill-input hide-bottom-space class="col-3" />
      ...
      <q-card v-if="gruping === '4.3' && !ulang" flat bordered class="q-mb-sm">
        <q-card-section>
          ...
          <q-separator class="q-my-xs" />
          <div class="row q-col-gutter-xs">
            <q-select v-model="store.formNeoNatal.crMasuk" dense standout="bg-yellow-3 text-black" outlined
              label="Cara Masuk" :options="store.caraMasuks" emit-value map-options input-class="ellipsis" fill-input
              hide-bottom-space class="col-4" />
            <q-select v-model="store.formNeoNatal.asalMasuk" dense standout="bg-yellow-3 text-black" outlined
              label="Asal Masuk" :options="store.asalMasuks" emit-value map-options input-class="ellipsis" fill-input
              hide-bottom-space class="col-8" />
            <app-input-simrs v-model="store.formNeoNatal.namaibu" label="Nama Ibu" class="col-6" />
            <app-input-simrs v-model="store.formNeoNatal.namaayah" label="Nama Ayah" class="col-6" />
            <app-input-simrs v-model="store.formNeoNatal.penanggungJawab" label="Nama Penanggung Jawab" class="col-6" />
            <app-input-simrs v-model="store.formNeoNatal.noHpPj" label="No Telp /HP penanggung jawab" class="col-6" />
            <app-input-simrs v-model="store.formNeoNatal.alamatPj" label="Alamat penanggung jawab" class="col-8" />
            <q-select v-model="store.formNeoNatal.hubPj" dense standout="bg-yellow-3 text-black" outlined
              label="Hub. penanggung jawab" :options="store.hubPj" hide-selected fill-input use-input input-debounce="0"
              hide-bottom-space hide-dropdown-icon @input-value="(val) => store.formNeoNatal.hubPj = val"
              class="col-4" />
          </div>
          <q-separator class="q-my-xs" />
          <div class="text-weight-bold">
          ...
          <div class="row q-col-gutter-xs">
            ...
            <q-select v-model="store.formNeoNatal.sgIbu" dense standout="bg-yellow-3 text-black" outlined
              label="Status Gizi (Ibu)" :options="store.baikBuruks" emit-value map-options input-class="ellipsis"
              fill-input hide-bottom-space class="col-4" />
            <app-input-simrs v-model="store.formNeoNatal.rwObat" label="Obat konsumsi slm (Hamil)" class="col-12" />
            <q-select v-model="store.formNeoNatal.kebiasaanIbu" dense standout="bg-yellow-3 text-black" outlined
              label="Kebiasaan (Ibu)" :options="store.kebiasaanIbus" hide-selected fill-input use-input
              input-debounce="0" hide-bottom-space hide-dropdown-icon
              @input-value="(val) => store.formNeoNatal.kebiasaanIbu = val" class="col-4" />
            <q-select v-model="store.formNeoNatal.rwPersalinan" dense standout="bg-yellow-3 text-black" outlined
              label="Riwayat Persalinan" :options="store.riwayatPersalinans" hide-selected fill-input use-input
              input-debounce="0" hide-bottom-space hide-dropdown-icon
              @input-value="(val) => store.formNeoNatal.rwPersalinan = val" class="col-4" />
            <q-select v-model="store.formNeoNatal.ketuban" dense standout="bg-yellow-3 text-black" outlined
              label="Ketuban" :options="store.ketubans" hide-selected fill-input use-input input-debounce="0"
              hide-bottom-space hide-dropdown-icon @input-value="(val) => store.formNeoNatal.ketuban = val"
              class="col-4" />
            <q-select v-model="store.formNeoNatal.volume" dense standout="bg-yellow-3 text-black" outlined
              label="Volume" :options="store.volumes" hide-selected fill-input use-input input-debounce="0"
              hide-bottom-space hide-dropdown-icon @input-value="(val) => store.formNeoNatal.volume = val"
              class="col-4" />
          </div>
          <q-separator class="q-my-xs" />
          <div class="text-weight-bold">
          ...
          <div class="row q-col-gutter-xs">
            ...
            <q-input v-model="store.formNeoNatal.reaksiTrans" outlined label="Reaksi Ketika Transfusi Darah"
              standout="bg-yellow-3" autogrow class="col-6" />
            <q-input v-model="store.formNeoNatal.rwImunisasi" outlined label="Riwayat Imunisasi" standout="bg-yellow-3"
              autogrow class="col-6" />
          </div>
          <q-separator class="q-my-xs" />
          <div class="text-weight-bold">
          ...
          <q-separator class="q-my-xs" />
          <div class="row q-col-gutter-xs">
            <q-select v-model="store.formNeoNatal.crLahir" dense standout="bg-yellow-3 text-black" outlined
              label="Cara Kelahiran" :options="store.caraLahirs" hide-selected fill-input use-input input-debounce="0"
              hide-bottom-space hide-dropdown-icon @input-value="(val) => store.formNeoNatal.crLahir = val"
              class="col-4" />
            <app-input-simrs v-model="store.formNeoNatal.apgarScore" label="APGAR Score" class="col-3"
              :valid="{ number: true }" />
            <q-select v-model="store.formNeoNatal.volumeKetuban" dense standout="bg-yellow-3 text-black" outlined
              label="Volume Air Ketuban" :options="store.volumes" hide-selected fill-input use-input input-debounce="0"
              hide-bottom-space hide-dropdown-icon @input-value="(val) => store.formNeoNatal.volumeKetuban = val"
              class="col-5" />
            <q-select v-model="store.formNeoNatal.warnaKetuban" dense standout="bg-yellow-3 text-black" outlined
              label="Warna Air Ketuban" :options="store.ketubans" hide-selected fill-input use-input input-debounce="0"
              hide-bottom-space hide-dropdown-icon @input-value="(val) => store.formNeoNatal.warnaKetuban = val"
              class="col-4" />
            ...
            <q-item>
              <q-item-section>
                <div class="row q-gutter-md items-center">
                  ...
                  <q-radio dense size="sm" v-model="store.formNeoNatal.golDarahAyah" v-for="aa in store.golDarahs"
                    :key="aa" :label="aa" :val="aa" />
                  <div>RH</div>
                  <q-radio dense size="sm" v-model="store.formNeoNatal.rhDarahAyah" v-for="aa in store.rhesus" :key="aa"
                    :label="aa" :val="aa" />
                </div>
              </q-item-section>
            </q-item>
            <q-item>
              <q-item-section>
                <div class="row q-gutter-md items-center">
                  ...
                  <q-radio dense size="sm" v-model="store.formNeoNatal.golDarahIbu" v-for="aa in store.golDarahs"
                    :key="aa" :label="aa" :val="aa" />
                  <div>RH</div>
                  <q-radio dense size="sm" v-model="store.formNeoNatal.rhDarahIbu" v-for="aa in store.rhesus" :key="aa"
                    :label="aa" :val="aa" />
                </div>
              </q-item-section>
            </q-item>
            ...
            <q-item>
              <q-item-section>
                <div class="row q-gutter-md items-center">
                  ...
                  <q-radio dense size="sm" v-model="store.formNeoNatal.golDarahBayi" v-for="aa in store.golDarahs"
                    :key="aa" :label="aa" :val="aa" />
                  <div>RH</div>
                  <q-radio dense size="sm" v-model="store.formNeoNatal.rhDarahBayi" v-for="aa in store.rhesus" :key="aa"
                    :label="aa" :val="aa" />
                </div>
              </q-item-section>
            </q-item>
      ...
      <q-card v-if="gruping === '4.4' && !ulang" flat bordered class="q-mb-sm">
        <q-card-section>
          ...
          <div class="row q-col-gutter-xs">
            ...
            <q-select v-model="store.formPediatrik.crKelahiran" dense standout="bg-yellow-3 text-black" outlined
              label="Cara Kelahiran" :options="store.caraKelahirans" hide-selected fill-input use-input
              input-debounce="0" hide-bottom-space hide-dropdown-icon
              @input-value="(val) => store.formPediatrik.crKelahiran = val" class="col-4" />
            <q-select v-model="store.formPediatrik.umurKelahiran" dense standout="bg-yellow-3 text-black" outlined
              label="Umur Kelahiran" :options="store.umurKelahirans" emit-value map-options input-class="ellipsis"
              fill-input hide-bottom-space class="col-4" />
          ...
          <div class="col-12 flex q-gutter-sm">
            <q-checkbox dense size="md" v-for="n in store.rwImunisasis" :key="n"
              v-model="store.formPediatrik.rwImunisasi" :label="n" :val="n" />
          </div>
          <q-separator class="q-my-sm" />
          <div class="text-weight-bold">
          ...
          <div class="row q-col-gutter-xs">
            ...
            <app-input-simrs v-model="store.formPediatrik.tdkSukaMknan" label="Makanan yg tidak disukai"
              class="col-6" />
            <q-select v-model="store.formPediatrik.nafsuMkn" dense standout="bg-yellow-3 text-black" outlined
              label="Nafsu Makan" :options="store.nafsuMakans" emit-value map-options input-class="ellipsis" fill-input
              hide-bottom-space class="col-4" />
            <q-select v-model="store.formPediatrik.polaMakan" dense standout="bg-yellow-3 text-black" outlined
              label="Pola Makan" :options="store.polaMakans" emit-value map-options input-class="ellipsis" fill-input
              hide-bottom-space class="col-4" />
            <q-select v-model="store.formPediatrik.mknYgdiberikan" dense standout="bg-yellow-3 text-black" outlined
              label="Mkanan yg diberikan" :options="store.makananYgdiberikans" hide-selected fill-input use-input
              input-debounce="0" hide-bottom-space hide-dropdown-icon
              @input-value="(val) => store.formPediatrik.mknYgdiberikan = val" class="col-4" />
          </div>
          <q-separator class="q-my-sm" />
          <div class="text-weight-bold">
            Pola Tidur
          ...
          <q-separator class="q-my-xs" />
          <div class="row q-col-gutter-xs">
            <q-select v-model="store.formPediatrik.kebiasaanSblmMkn" dense standout="bg-yellow-3 text-black" outlined
              label="Kebiasaan sblm makan" :options="store.kebiasaanSblMakans" hide-selected fill-input use-input
              input-debounce="0" hide-bottom-space hide-dropdown-icon
              @input-value="(val) => store.formPediatrik.kebiasaanSblmMkn = val" class="col-6" />
            <q-select v-model="store.formPediatrik.nyeri" dense standout="bg-yellow-3 text-black" outlined
              label="Adakah nyeri" :options="store.yaTidaks" hide-selected fill-input use-input input-debounce="0"
              hide-bottom-space hide-dropdown-icon @input-value="(val) => store.formPediatrik.nyeri = val"
              class="col-6" />
          </div>
          ...
          <div class="row q-col-gutter-sm ">
            ...
            <q-select v-model="store.formPediatrik.kbersihanKuku" dense standout="bg-yellow-3 text-black" outlined
              label="Kebersihan Kuku" :options="['Bersih', 'Kotor']" emit-value map-options input-class="ellipsis"
              fill-input hide-bottom-space class="col-4" />
          </div>
          <q-separator class="q-my-xs q-mb-sm" />
          <div class="row q-col-gutter-xs">
            <q-select v-model="store.formPediatrik.aktifitas" dense standout="bg-yellow-3 text-black" outlined
              label="Aktifitas Bermain" :options="store.aktifitasBermains" emit-value map-options fill-input
              hide-bottom-space class="col-4" />
