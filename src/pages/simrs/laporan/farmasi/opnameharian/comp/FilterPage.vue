      ...
      <q-card-section class="q-pa-sm">
        <div class="row q-col-gutter-sm items-center">
          ...
          <div class="col-6 col-md-2">
            <app-input-date :model="store.params.tanggal" outlined @setModel="(val) => {
              store.params.tanggal = val
            }" />
          </div>
          <!-- <div class="col-6 col-md-2">
          ...
          <div class="col-12 col-md-1 text-right">
            <app-btn label="Ambil Data" :disable="store.loading || !!store.ketProses" :loading="store.loading"
              @click="store.getInitialData(1)" dense />
          </div>
        </div>
      </q-card-section>
      ...
      <q-card-section class="q-pa-sm row justify-between items-center bg-grey-1">
        <div class="row items-center q-gutter-sm">
          ...
          <template v-if="store.items.length > 0">
            <q-input v-model="store.filters.q" dense outlined placeholder="Cari obat di tabel..." style="width: 200px"
              bg-color="white">
            ...
            <q-checkbox v-model="store.filters.hasActivity" label="Hanya yg ada aktifitas" dense color="primary" />
          </template>
        </div>
