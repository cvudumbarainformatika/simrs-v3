  ...
  <q-form ref="refForm" class="bg-grey-1 q-pa-sm" style="display: flex; flex-direction: column">
      ...
      <q-card-section>
        ...
        <div class="row q-mt-sm">
          <div class="col-12 text-weight-bold">
            Jenis Pelayanan :
            <q-badge :color="paymentOptions.find(
              (item) => item.value === storex.jenispembayaran
            )?.color
              ">{{
                storex.jenispembayaran === "" ? "Semua" : storex.jenispembayaran
              }}</q-badge>
          </div>
        </div>
      </q-card-section>
    ...
    <q-card class="rounded-borders shadow-2" style="height: 500px; display: flex; flex-direction: column">
      ...
      <q-scroll-area class="col-grow">
        ...
        <div v-else style="flex: 1; overflow-y: auto; padding-bottom: 12px">
          <div class="col-12 q-mt-sm" style="overflow-y: auto; padding-bottom: 50px">
            <div class="row q-mt-sm" v-if="
              (selectedPayment === '' || selectedPayment === 'obat') &&
              store.itemsobat
            ">
              <q-list class="full-width kwitansi-card" v-for="(item, x) in store.itemsobat" :key="x">
                <q-item>
                  <q-item-section>
                    ...
                    <q-item-label>
                      Subtotal Pelayanan Obat
                      <q-badge :color="paymentOptions.find((item) => item.value === 'obat')
                          ?.color
                        " class="text-weight-bold">{{ formatRpDouble(item?.subtotal) }}</q-badge></q-item-label>
                  </q-item-section>
                  <q-item-section side top>
                    ...
                    <q-btn v-if="store.form.carabayar === 'Tunai'" unelevated color="dark" round size="sm"
                      icon="icon-mat-attach_money" @click="
                        bayarnonkarcis(item?.nota, item?.subtotal, 'Farmasi')
              ...
              <q-list class="full-width kwitansi-card" v-for="(item, b) in store.itemslaborat" :key="b">
                <q-item>
                  <q-item-section>
                    <q-item-label>Nota : {{ item?.nota }}</q-item-label>
                    <q-item-label>
                      Subtotal Pelayanan Laborat
                      <q-badge :color="paymentOptions.find(
                        (item) => item.value === 'Laboratorium'
                      )?.color
                        " class="text-weight-bold">{{ formatRpDouble(item?.total_subtotal) }}</q-badge></q-item-label>
                  </q-item-section>
            ...
            <div class="row q-mt-sm" v-if="
              (selectedPayment === '' || selectedPayment === 'Radiologi') &&
              store.itemsradiologi
            ">
              <q-list class="full-width kwitansi-card" v-for="(item, c) in store.itemsradiologi" :key="c">
                <q-item>
                  <q-item-section>
                    ...
                    <q-item-label>
                      Subtotal Pelayanan Radiologi
                      <q-badge :color="paymentOptions.find(
                        (item) => item.value === 'Radiologi'
                      )?.color
                        " class="text-weight-bold">{{ formatRpDouble(item?.total) }}</q-badge></q-item-label>
                  </q-item-section>
                  ...
                  <q-item-section side top>
                    ...
                    <q-btn v-if="store.form.carabayar === 'Tunai'" unelevated color="dark" round size="sm"
                      icon="icon-mat-attach_money" @click="
                        bayarnonkarcis(item?.rs2, item?.total, 'Radiologi')
                        " :loading="store.loadingpembayaran">
                      <q-tooltip class="primary" :offset="[10, 10]">
                        Bayar & Print
                      </q-tooltip>
      ...
      <div
        ...
        <span>Total :</span>
        <span v-if="store.loadingbayar === false">{{ formatRpDouble(totalbill) }}
        </span>
...
const totalbill = computed(() => {
  ...
  const totalobat = parseFloat(
    store.itemsobat?.reduce((acc, cur) => acc + cur.subtotal, 0) ?? 0
  );
  ...
  const totallaborat = parseFloat(
    store.itemslaborat?.reduce((acc, cur) => acc + cur.total_subtotal, 0) ?? 0
  );
  const totalradiologi = parseFloat(
    store.itemsradiologi?.reduce((acc, cur) => acc + cur.total, 0) ?? 0
  );
  ...
  const total = parseFloat(totalobat + totallaborat + totalradiologi);
  store.total = total;
  return total;
});
...
function caripembayaran() {
  if (storex.jenispembayaran === "") {
    $q.notify({
      ...
      title: "Peringatan",
      message:
        "INFO WARNING <b/> MAAF,<em><b>Anda Belum Memilih Jenis Pelayanan</b></em>",
      html: true,
      timeout: 1000,
    });
  } else {
...
function bayarnonkarcis(nota, total, jenis) {
  if (store.form.carabayar === "" || store.form.carabayar === "") {
    $q.notify({
      ...
      title: "Peringatan",
      message:
        "INFO WARNING <b/> MAAF,<em><b>Anda Belum Memilih Cara Bayar</b></em>",
      html: true,
      timeout: 1000,
    });
  } else {
...
onMounted(() => {
  ...
  store.carilaborat(prop.pasien?.noreg, "luar");
  store.cariradiologi(prop.pasien?.noreg, "luar");
