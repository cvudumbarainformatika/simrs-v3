<template>
  ...
  <div class="q-mx-sm">
    <app-table-extend :columns="store.columns" :column-hide="store.columnHide" :items="store.items" :meta="store.meta"
      :per-page="store.param.per_page" :loading="store.loading" :to-search="store.param.nopemesanan" :click-able="true"
      ...
      <template #header-left-after-search>
        <div class="row items-center">
          <q-input v-model="store.param.obat" class="q-ml-sm" outlined dark color="white" dense
            placeholder="Cari Obat ..." debounce="500" @update:model-value="store.setSearchObat" />
          <div class="q-ml-xs text-white">
            <q-option-group v-model="store.param.gudang" :options="store.gudangs" color="primary" class="q-ml-sm" dense
              type="checkbox" inline @update:model-value="store.setGudang" />
          </div>
        </div>
      ...
      <template #expand="{ row }">
        <div v-if="row.rinci?.length">
          ...
          <div v-for="(rin, i) in row.rinci" :key="i" :class="rin.flag === '2' ? 'bg-red-2' : ''">
            <div class="row items-center anu">
              ...
              <div class="col-1 text-right">
                <q-btn v-if="!row.flag" class="q-mr-md" flat icon="icon-mat-delete" size="sm" no-caps color="negative"
                  :loading="pemesanan.loading && rin.loading" @click="batalRinci(rin)">
                  <q-tooltip class="primary" :offset="[10, 10]">
                    Batalkan Rincian Pesanan
                  </q-tooltip>
                </q-btn>
                <div v-if="rin.flag === '2'" class="text-weight-bold f-18 text-negative">
                  DITOLAK
                </div>
              </div>
            </div>
      ...
      <template #left-acttion="{ row }">
        <div v-if="!row.flag">
          <q-btn class="q-mr-md" flat icon="icon-mat-add_circle" dense no-caps color="primary" @click="tambah(row)">
            <q-tooltip class="primary" :offset="[10, 10]">
              Tambah Pesanan
            </q-tooltip>
          </q-btn>
          <q-btn class="q-mr-md" flat icon="icon-mat-delete" dense no-caps color="negative"
            :loading="pemesanan.loading && row.loading" @click="batal(row)">
            <q-tooltip class="primary" :offset="[10, 10]">
              Batalkan Pesanan
            </q-tooltip>
          </q-btn>
          <q-btn flat icon="icon-mat-lock_open" dense color="green"
            :loading="pemesanan.loading && row.nopemesanan === toloadBeli" @click="kunci(row)">
            <q-tooltip class="primary" :offset="[10, 10]">
        ...
        <div v-if="row.flag">
          <q-btn v-if="row.flag === '1'" icon="icon-mat-done_all" dense color="negative" :loading="row.loading"
            :disable="row.loading" @click="dianggapSelesai(row)">
            <q-tooltip class="primary" :offset="[10, 10]">
    ...
    <app-fullscreen-blue v-model="pemesanan.isOpen" @close="pemesanan.setClose">
      <template #default>
        <CompDialog />
      </template>
    </app-fullscreen-blue>
    <CetakPemesananPage ref="refCetakPemesanan" v-model="printCetakPemesanan" />
  </div>
</template>
<script setup>
