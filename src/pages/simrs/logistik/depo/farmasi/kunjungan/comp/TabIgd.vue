<template>
  <div>
    ...
    <div v-if="loading" class="column flex-center q-pa-lg">
      ...
      <div class="q-mt-sm text-grey-7">Memuat data kunjungan IGD...</div>
    </div>

    <!-- Empty State -->
    <div v-else-if="!items?.length" class="column flex-center q-pa-lg">
      <q-icon name="group_off" size="50px" color="grey-5" />
      <div class="q-mt-sm text-grey-7">Tidak ada data kunjungan IGD ditemukan.</div>
    </div>
    ...
    <q-list v-else separator class="q-pb-xl">
      <q-item v-for="(item, i) in items" :key="i" class="hover-row">
        ...
        <q-item-section class="q-col-gutter-xs">
          ...
          <q-item-label caption>
            Status Pelayanan : <q-badge :color="item.status === '1' ? 'green' : 'orange'" outline>{{ item.status === '1' ? 'SELESAI PELAYANAN' : 'AKTIF' }}</q-badge>
          </q-item-label>
        </q-item-section>
        ...
        <q-item-section class="q-col-gutter-xs">
          ...
          <q-item-label>
            RUANGAN : <span class="text-teal text-weight-bold"> {{ item.poli ?? 'IGD / IRD' }}</span>
          </q-item-label>
          <q-item-label>
