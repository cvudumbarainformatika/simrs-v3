<template>
  <div class="column" style="min-height: 60px;">
    <div class="row items-center" style="height: 60px;">
      <div class="col-4">
        <div class="flex items-center">
          <q-icon :name="`${store.pasien ? 'icon-mat-person' : 'icon-mat-bedroom_parent'}`" size="md" color="grey-8" />
          <div class="q-ml-sm">
            <div class="f-18">
      ...
      <div class="col-8">
        <div v-if="store.pasien" class="full-width flex items-center q-gutter-sm justify-end">
          <q-btn flat color="grey-8" padding="xs" size="md" round @click="store.pasien = null">
            <q-icon name="icon-mat-close" />
            <q-tooltip>Refresh</q-tooltip>
          </q-btn>
        </div>
        <div v-else class="full-width flex items-center q-gutter-sm justify-end">
          <div>
            <q-input v-model="store.params.q" placeholder="Cari" class="bg-white" dense outlined
              ...
              <template v-if="store.params.q" #append>
                <div class="cursor-pointer">
                  <q-icon name="icon-mat-close" size="xs" @click="clearQ" />
                </div>
              </template>
            </q-input>
          </div>
          <q-btn outline color="grey-8" class="bg-white" padding="sm" size="sm" no-caps>
            <div class="flex">
              <transition>
                <q-icon v-if="store.isViewList" name="icon-mat-dataset" size="16px" />
                <q-icon v-else name="icon-mat-format_list_numbered" size="16px" />
              </transition>
              <div class="relative-position" style="width: 45px;">
                <!-- <div class="absolute-top-right"> -->
                <q-toggle v-model="store.isViewList" size="sm" class="absolute" style="top: -10px; right: 0;" />
                <!-- </div> -->
              </div>
            </div>
          </q-btn>
              ...
              <div class="row no-wrap q-pa-sms">
                ...
                <div class="column">
                  ...
                  <div class="row q-pa-sm justify-end">
                    <q-btn v-close-popup color="primary" label="Terapkan" push size="sm" @click="store.getDataTable" />
                  </div>
                </div>
              </div>
            </q-menu>
          ...
          <!-- blm jls -->
          <q-btn v-for="(item, i) in store.statuses" :key="i" outline
            :color="`${item === store.params.status ? 'white' : 'grey-8'}`" :class="`${item === store.params.status ? i === 0 ? 'bg-grey-8' : (i === 1 ? 'bg-teal' : i === 3 ? 'bg-negative' : 'bg-deep-orange-7')
              : 'bg-white'}`" padding="sm" size="sm" @click="store.setStatus(item)">
            <q-icon
              :name="`${i === 0 ? 'icon-cus-groups' : (i === 1 ? 'icon-mat-done_all' : i === 3 ? 'icon-mat-error' : 'icon-mat-access_time')}`" />

            <q-tooltip>{{ item }}</q-tooltip>
          </q-btn>

          <q-btn outline color="grey-8" class="bg-white" padding="sm" size="sm" @click="store.getDataTable">
    ...
    <q-separator />
    <div v-if="!store.pasien" class="row items-center q-px-sm q-py-xs">
      <div class="col-6">
          ...
          <div class="cursor-pointer">
            ...
            <q-menu>
              <q-list style="min-width: 100px" dense separator>
                <q-item v-for="item in store.sorting" :key="item" v-close-popup clickable
