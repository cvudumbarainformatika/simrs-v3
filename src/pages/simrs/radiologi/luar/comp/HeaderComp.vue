    ...
    <div class="row items-center" style="height: 60px;">
      <div class="col-4">
        <div class="flex items-center">
          <q-icon :name="`${store.pasien ? 'icon-mat-person' : 'icon-mat-add_shopping_cart'}`" size="md"
            color="grey-8" />
          <div class="q-ml-sm">
            <div class="f-18">
      ...
      <div class="col-8">
        ...
        <div v-else class="full-width flex items-center q-gutter-sm justify-end">
          ...
          <q-btn v-for="(item, i) in store.statuses" :key="i" outline
            :color="`${item === store.params.status ? 'white' : 'grey-8'}`" :class="`${item === store.params.status ? i === 0 ? 'bg-grey-8' : (i === 1 ? 'bg-teal' : 'bg-deep-orange-8')
