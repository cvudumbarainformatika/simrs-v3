        ...
        <q-expansion-item v-for="(item, i) in filteredItems" :key="i" expand-separator
          ...
          <q-card dark>
            ...
            <q-list separator>
              ...
              <div v-for="(pemeriksaanfisik, f) in item?.pemeriksaanfisik" :key="f">
                ...
                <q-item v-ripple class="q-pa-xs list-move" dark clickable>
                  <q-item-section avatar>
                    <q-icon :color="nadi(pemeriksaanfisik?.rs4).color" name="icon-my-monitor_heart" size="lg" />
                  </q-item-section>
                  ...
                  <q-item v-for="(tindakan, tin) in item?.tindakan" :key="tin" v-ripple clickable>
                    <q-item-section>
                      ...
                      <div v-if="tindakan?.gambardokumens?.length > 0" class="image-row">
                        <div v-for="(image, index) in tindakan?.gambardokumens" :key="index" class="image-container">
              ...
              <q-card-section v-if="item?.transradiologi?.length && item?.hasilradiologi?.length" class="q-pa-none" flat
                bordered square dark>
                <q-bar class="bg-accent">
                  <div>RADIOLOGI</div>
                </q-bar>
                <q-list dark separator>
                ...
                <q-markup-table separator="vertical" flat bordered dense dark>
                  ...
                  <tbody>
                    <tr v-for="(apotekrajal, wew) in item?.apotekrajal" :key="wew">
                      ...
                      <td v-if="apotekrajal?.rs27?.length > 0 && apotekrajal?.rs28?.length > 0" class="text-right"
                    ...
                    <tr v-for="(apotekranapracikanhederlalu, lxxx) in item?.apotekranapracikanhederlalu" :key="lxxx">
                      <td class="text-left ellipsis" style="max-width: 250px;">
                        ...
                        <div class="absolute-bottom">
                          <div class="column items-center justify-between">
                            <q-btn class="gt-xs" size="md" color="yellow" flat dense round icon="icon-mat-visibility"
                              :href="pathImg + image?.url" target="_blank" />
                          </div>
