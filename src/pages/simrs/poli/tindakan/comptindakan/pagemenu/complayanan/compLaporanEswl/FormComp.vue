<template>
  <q-form ref="myForm" class="column full-height" style="overflow: hidden;">
    ...
    <q-scroll-area class="col q-px-md q-py-sm">
      <div class="row q-col-gutter-md q-mb-md">
        ...
        <div class="col-md-6 col-sm-12">
          <app-input-date :model="store.form.tanggal" label="Tanggal Tindakan" icon="icon-mat-event" outlined
            @set-model="val => store.form.tanggal = val" />
        </div>

        <div class="col-md-6 col-sm-12">
          <app-autocomplete-new :model="store.form.kddokter" label="Dokter Operator (DPJP)" autocomplete="nama"
            option-value="kode" option-label="nama" outlined :source="store.listdokters"
            @on-select="(val) => store.form.kddokter = val" @clear="() => store.form.kddokter = null" />
        </div>

        <div class="col-md-6 col-sm-12">
          <app-autocomplete-new :model="store.form.asisten" label="Asisten Dokter (Perawat)" autocomplete="nama"
            option-value="kode" option-label="nama" outlined :source="store.listperawats"
        ...
        <div class="col-md-3 col-sm-6">
          <q-input v-model="store.form.waktu_mulai" label="Waktu Mulai" outlined dense type="time"
            standout="bg-yellow-3 text-black" @update:model-value="calculateDuration" />
        </div>

        <div class="col-md-3 col-sm-6">
          <q-input v-model="store.form.waktu_selesai" label="Waktu Selesai" outlined dense type="time"
            standout="bg-yellow-3 text-black" @update:model-value="calculateDuration" />
        </div>
        ...
        <div class="col-md-3 col-sm-6">
          <q-select v-model="store.form.posisi" label="Posisi Pasien" outlined dense :options="['Prone', 'Supine']"
            standout="bg-yellow-3 text-black" />
        </div>
        ...
        <div class="col-12 row q-col-gutter-sm bg-grey-1 q-py-sm rounded-borders border-grey-4">
          <div v-for="(batu, idx) in store.form.batu_detail" :key="idx" class="col-md-6 col-sm-12 row q-col-gutter-sm">
            ...
            <div class="col-6">
              <q-input v-model="batu.posisi" label="Posisi Batu" outlined dense placeholder="misal: Renal Sinistra"
                standout="bg-yellow-3 text-black" />
            </div>
            <div class="col-3">
              <q-input v-model="batu.ukuran_x" label="Pjg (mm)" outlined dense type="number"
                standout="bg-yellow-3 text-black" />
            </div>
            <div class="col-3">
              <q-input v-model="batu.ukuran_y" label="Lbr (mm)" outlined dense type="number"
                standout="bg-yellow-3 text-black" />
            </div>
          </div>
        </div>
        ...
        <div class="col-md-4 col-sm-6">
          <q-select v-model="store.form.lokalisasi_type" label="Tipe Lokalisasi" outlined dense
            :options="['X-Ray', 'Ultrasound', 'Keduanya']" standout="bg-yellow-3 text-black" />
        </div>

        <div class="col-md-4 col-sm-6">
          <q-input v-model.number="store.form.lokalisasi_lama" label="Lama Lokalisasi (menit)" outlined dense
            type="number" standout="bg-yellow-3 text-black" />
        </div>

        <div class="col-md-4 col-sm-12">
          <q-select v-model="store.form.lokalisasi_gambar" label="Kualitas Gambar" outlined dense
            :options="['Bagus', 'Cukup bagus', 'Tidak bagus']" standout="bg-yellow-3 text-black" />
        </div>

        <!-- Conditional Lokalisasi Details -->
        <!-- X-Ray fields -->
        <div v-if="store.form.lokalisasi_type === 'X-Ray' || store.form.lokalisasi_type === 'Keduanya'"
          class="col-12 row q-col-gutter-sm bg-blue-1 rounded-borders q-py-xs">
          <div class="col-12 text-bold text-caption text-blue-9">Detail Lokalisasi X-Ray</div>
          <div class="col-md-4 col-sm-4">
            <q-input v-model="store.form.lokalisasi_xray.kv" label="KV" outlined dense
              standout="bg-yellow-3 text-black" />
          </div>
          <div class="col-md-4 col-sm-4">
            <q-input v-model="store.form.lokalisasi_xray.ma" label="mA" outlined dense
              standout="bg-yellow-3 text-black" />
          </div>
          <div class="col-md-4 col-sm-4">
            <q-input v-model.number="store.form.lokalisasi_xray.fluroscopy" label="Fluroscopy (menit)" outlined dense
              type="number" standout="bg-yellow-3 text-black" />
          </div>
        </div>

        <!-- Ultrasound (USG) fields -->
        <div v-if="store.form.lokalisasi_type === 'Ultrasound' || store.form.lokalisasi_type === 'Keduanya'"
          class="col-12 row q-col-gutter-sm bg-green-1 rounded-borders q-py-xs q-mt-xs">
          <div class="col-12 text-bold text-caption text-green-9">Detail Lokalisasi Ultrasound (U/S)</div>
          <div class="col-md-4 col-sm-4">
            <q-input v-model.number="store.form.lokalisasi_usg.probe_fokus" label="Jarak Probe-Fokus (mm)" outlined
              dense type="number" standout="bg-yellow-3 text-black" />
          </div>
          <div class="col-md-4 col-sm-4">
            <q-input v-model.number="store.form.lokalisasi_usg.probe_batu" label="Jarak Probe-Batu (mm)" outlined dense
              type="number" standout="bg-yellow-3 text-black" />
          </div>
          <div class="col-md-4 col-sm-4">
            <q-input v-model="store.form.lokalisasi_usg.perputaran" label="Perputaran U/S" outlined dense
              standout="bg-yellow-3 text-black" />
          </div>
        </div>
        ...
        <div class="col-12 q-my-md">
          <div class="row justify-between items-center q-mb-xs">
            ...
            <q-btn label="Reset Coretan" icon="icon-mat-refresh" size="sm" color="negative" outline
              @click="resetCanvas" />
          </div>
          <div class="canvas-container flex flex-center bg-grey-2 q-pa-sm rounded-borders relative-position"
            style="max-width: 100%;">
            <div class="relative-position shadow-1 bg-white"
              style="width: 100%; max-width: 500px; overflow: hidden; flex-shrink: 0;">
              <!-- Background Template -->
              <img :src="bodyMarkerImg" style="display: block; width: 100%; height: auto; pointer-events: none;" />

              <!-- Drawing Canvas (Transparent) -->
              <canvas ref="canvasRef" width="1024" height="601"
          ...
          <div class="text-caption text-grey-7 q-mt-xs">
            *Gunakan mouse atau jari Anda (di tablet/layar sentuh) untuk mencoret atau melingkari posisi batu pada
            diagram saluran kemih di atas.
          </div>
        </div>
        ...
        <div class="col-md-4 col-sm-6">
          <q-select v-model="store.form.sinkronisasi" label="Sinkronisasi" outlined dense
        ...
        <div class="col-12">
          ...
          <div class="row q-col-gutter-sm bg-grey-1 q-py-sm rounded-borders">
            <div v-for="(detail, index) in store.form.penembakan_detail" :key="index"
              ...
              <div class="col-md-3 col-sm-4">
                <q-input v-model.number="detail.tembakan" label="Jml Tembakan" outlined dense type="number"
                  standout="bg-yellow-3 text-black" />
              </div>
              <div class="col-md-3 col-sm-4">
                <q-input v-model="detail.power" label="Power (kV)" outlined dense standout="bg-yellow-3 text-black" />
              </div>
              <div class="col-md-3 col-sm-4">
                <q-input v-model="detail.energi" label="Energi (Hz)" outlined dense standout="bg-yellow-3 text-black" />
              </div>
            </div>
          </div>
        </div>
        ...
        <div class="col-md-6 col-sm-12 row q-col-gutter-sm bg-yellow-1 rounded-borders q-py-xs">
          <div class="col-4">
            <q-select v-model="store.form.monitor_usg" label="Monitor USG" outlined dense :options="['Ya', 'Tidak']"
              standout="bg-yellow-3 text-black" />
          </div>
          <div class="col-8">
            <q-input v-model="store.form.monitor_usg_catatan" label="Catatan USG" outlined dense
              :disable="store.form.monitor_usg === 'Tidak'" standout="bg-yellow-3 text-black" />
          </div>
        </div>

        <div class="col-md-6 col-sm-12 row q-col-gutter-sm bg-orange-1 rounded-borders q-py-xs">
          <div class="col-4">
            <q-select v-model="store.form.monitor_rontgen" label="Monitor Rontgen" outlined dense
              :options="['Ya', 'Tidak']" standout="bg-yellow-3 text-black" />
          </div>
          <div class="col-8">
            <q-input v-model="store.form.monitor_rontgen_catatan" label="Catatan Rontgen" outlined dense
              :disable="store.form.monitor_rontgen === 'Tidak'" standout="bg-yellow-3 text-black" />
          </div>
        </div>
        ...
        <div class="col-md-4 col-sm-12">
          <q-input v-model="store.form.obat_pre" label="Sebelum Penembakan (Pre)" outlined type="textarea" rows="2"
            standout="bg-yellow-3 text-black" />
        </div>

        <div class="col-md-4 col-sm-12">
          <q-input v-model="store.form.obat_durante" label="Selama Penembakan (Durante)" outlined type="textarea"
            rows="2" standout="bg-yellow-3 text-black" />
        </div>

        <div class="col-md-4 col-sm-12">
          <q-input v-model="store.form.obat_post" label="Setelah Penembakan (Post)" outlined type="textarea" rows="2"
            standout="bg-yellow-3 text-black" />
        </div>
        ...
        <div class="col-md-4 col-sm-6">
          <q-select v-model="store.form.kepecahan_batu" label="Tingkat Kepecahan Batu" outlined dense
            :options="['Terlihat jelas', 'Terlihat samar', 'Tidak terlihat']" standout="bg-yellow-3 text-black" />
        </div>

        <div class="col-md-4 col-sm-6">
          <q-input v-model.number="store.form.lama_penembakan" label="Lama Penembakan (menit)" outlined dense
            type="number" standout="bg-yellow-3 text-black" />
        </div>

        <div class="col-12">
          <q-input v-model="store.form.keterangan" label="Catatan / Keterangan" outlined type="textarea" rows="2"
            standout="bg-yellow-3 text-black" />
        </div>
    ...
    <q-card-actions align="right" class="bg-grey-2 q-pa-md border-top-grey-4">
      <q-btn v-if="store.form.id" label="Batal Edit" color="dark" flat class="q-mr-sm" @click="store.initForm" />
      <q-btn label="Simpan Laporan ESWL" color="primary" icon="icon-mat-save" :loading="store.loading"
        :disable="store.loading" @click="submitForm" />
    </q-card-actions>
  </q-form>
</template>
