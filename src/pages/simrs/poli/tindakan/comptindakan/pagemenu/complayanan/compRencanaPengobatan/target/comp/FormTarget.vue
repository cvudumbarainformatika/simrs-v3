  ...
  <q-form class="fit q-pa-lg bg-grey-3 scroll" @submit="onSubmit">
    ...
    <div class="row q-col-gutter-md">
      <div class="col-4">
        <q-card>
          <q-card-section>
            <q-input v-model="target" type="textarea" label="Target Keberhasilan" outlined standout="bg-yellow-3"
              rows="8" :rules="[val => !!val?.length || 'Harap diisi']" hide-bottom-space />
          </q-card-section>
        </q-card>
      </div>
      <div class="col-4">
        <q-card>
          <q-card-section>
            <q-input v-model="terapi" type="textarea" label="Rencana Terapi yg diberikan" outlined
              standout="bg-yellow-3" rows="8" :rules="[val => !!val?.length || 'Harap diisi']" hide-bottom-space />
          </q-card-section>
        </q-card>
      </div>
      <!-- <div class="col-4">
        <q-card>
          <q-card-section>
            <q-input v-model="monitor" type="textarea" label="Monitoring Pasien" outlined standout="bg-yellow-3"
              rows="8" :rules="[val => !!val?.length || 'Harap diisi']" hide-bottom-space />
          </q-card-section>
        </q-card>
      </div> -->
    ...
    <div class="text-right">
      <q-btn :loading="loading" :disable="loading" label="Simpan Planning Awal" color="primary" class="q-px-md"
        type="submit" />
    </div>
  </q-form>
</template>
...
const onSubmit = async () => {
  ...
  // return console.log('plaintext', props?.pasien)
  loading.value = true
  try {
    const form = {
    ...
    if (resp.status === 200) {
      ...
      const findPasien = pengunjung?.items.find(x => x?.noreg === props?.pasien?.noreg)
      if (findPasien) {
