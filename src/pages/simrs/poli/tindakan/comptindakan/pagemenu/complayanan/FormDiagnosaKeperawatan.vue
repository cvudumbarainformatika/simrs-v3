<template>
  <q-form class="column full-height full-width" @submit="simpan">
    <div class="col-auto">
      ...
      <div class="q-px-md">
        <q-input v-model="store.diagnosa" label="Diagnosa Keperawtan (Klik Untuk Cari Diagnosa)" autogrow outlined
          standout="bg-yellow-3" icon="icon-mat-search" :rules="[val => !!val || 'Harap cari Diagnosa dahulu']"
          lazy-rules="ondemand" hide-bottom-space @click="modalOpen = true" @update:model-value="lihatDiagnosa" />
      </div>
    ...
    <div class="col full-height q-px-md">
      ...
      <q-scroll-area style="height:calc(100% - 20px)">
        ...
        <q-separator class="q-my-lg" style="margin-bottom: 30px;" />
    ...
    <!-- modal diagnosa -->
    <modal-diagnosa-keperawatan :key="props?.pasien" v-model="modalOpen" :masters="store.diagnosas"
      @ok="modalOpen = false" />
  </q-form>
</template>
