        ...
        <div id="pdfDoc" ref="rePdfDoc" class="page-legal f-12 bg-white shadow-2">
            <div class="contentx">
                ...
                <div class="dokumen-content q-mb-md">
                    <div v-if="!konsultasis || konsultasis.length === 0"
                        class="text-center q-pa-xl text-grey border-box">
                    ...
                    <div v-else>
                        <div v-for="(item, index) in konsultasis" :key="item.id" class="q-mb-xl p-box"
                            ...
                            <table class="doc-table q-mt-sm">
                                <thead>
                                    <th colspan="2" class="bg-grey-2 text-weight-bold">I. PERMINTAAN KONSUL (PENGANTAR)
                                    </th>
                                </thead>
                                <tbody>
                                    <tr>
                                        <td width="30%" class="text-weight-bold">Kepada Yth. Dokter</td>
                                        <!-- <td>: <strong>{{ item?.dokterkonsul?.nama || namaPetugas(item?.kddokterkonsul)
                                                }}</strong></td> -->
                                        <td>: <strong>{{ namaPetugas(item?.pemberi_konsul?.rs9) ||
                                            namaPetugas(item?.pemberi_konsul?.rs9) }}</strong></td>
                                    </tr>
                                    <!-- <tr>
                                    ...
                                    <tr>
                                        ...
                                        <td>: {{ dateFullFormat(item?.created_at) }} Jam {{
                                            jamTnpDetik(item?.created_at) }}</td>
                                    </tr>
                                    <tr>
                                        <td class="text-weight-bold">Peminta Konsul</td>
                                        <td>: {{ namaPetugas(item?.peminta_konsul?.rs9) ?? '-' }}</td>
                                    </tr>
                                </tbody>
                            </table>
                                ...
                                <tbody>
                                    <template v-if="item?.jawaban">
                                        ...
                                        <tr>
                                            ...
                                            <td>: {{ dateFullFormat(item?.updated_at) }} Jam {{
                                                jamTnpDetik(item?.updated_at) }}</td>
                                        </tr>
                                        <tr>
                                            <td class="text-weight-bold">Dokter Konsulan</td>
                                            <td>: {{ item?.dokterkonsul?.nama ||
                                                namaPetugas(item?.pemberi_konsul?.rs9) }}
                                            </td>
                                        </tr>
                                        <tr>
                                            ...
                                            <td>
                                                <q-badge
                                                    :color="item?.pemberi_konsul?.rs9 === item?.pemberi_konsul?.rs9 ? 'primary' : 'orange-9'"
                                                    text-color="white" class="q-px-sm">
                                                    {{ item?.pemberi_konsul?.rs9 === item?.pemberi_konsul?.rs9 ?
                                                        'Terverifikasi Dokter Konsul' : 'Belum Terverifikasi Dokter Konsul'
                                                    }}
                                                </q-badge>
                                            </td>
                                        </tr>
                            ...
                            <div class="row q-mt-md justify-between"
                                ...
                                <div class="col-5 text-center">
                                    <div class="text-weight-bold q-mb-xs f-10">
                                        {{ item?.peminta_konsul?.petugas?.kdgroupnakes === '1' ?
                                            'Dokter Peminta Konsul' : 'Nakes Peminta Konsul' }}
                                    </div>
                                    <div v-if="qrUrl(item, 'permintaan')"
                                        style="height: 75px; width: 75px; margin: 0 auto;" class="q-mb-xs">
                                    ...
                                    <div style="text-decoration: underline;" class="text-weight-bold f-11">
                                        {{ namaPetugas(item?.peminta_konsul?.rs9) ??
                                            '..........................................' }}
                                    </div>
                                    <div class="f-9">{{ formatNipOrId(item?.peminta_konsul?.rs9,
                                        false) }}</div>
                                </div>

                                <div class="col-5 text-center" v-if="item?.jawaban">
                                    <div class="text-weight-bold q-mb-xs f-10">Dokter Konsulan</div>
                                    <div v-if="qrUrl(item, 'jawaban')"
                                        style="height: 75px; width: 75px; margin: 0 auto;" class="q-mb-xs">
                                    ...
                                    <div class="f-9">{{ formatNipOrId(item?.pemberi_konsul?.rs9, true) }}
                                    </div>
                                </div>
                            </div>
                        </div>
...
function namaPetugas(code) {
    // console.log('namaPetugas code', code)
    // console.log('store.dokters', store.dokters)
    if (!code) return '-'
...
function qrUrl(item, type) {
    ...
    const dok = type === 'jawaban' ? 'JAWABAN KONSUL.png' : 'PERMINTAAN KONSUL.png'
    const asal = 'PENUNJANG'
    const petugas = type === 'jawaban'
        ? (item?.pemberi_konsul?.rs9 || item?.kddokterkonsul)
        : (item?.peminta_konsul?.rs9 || item?.kdminta || props.pasien?.kodedokter)
    if (!petugas) return ''
