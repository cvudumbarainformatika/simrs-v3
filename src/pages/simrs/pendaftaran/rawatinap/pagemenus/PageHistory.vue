<template>
  <div class="fit">
    <div class="fit column relative-position">
      ...
      <div class="col full-height">
        <template v-if="store.items?.length > 0">
          <transition-group appear enter-active-class="animated fadeIn faster"
            ...
            <list-pasien v-if="!store.isViewList" key="list-pasien" :items="store.items" :loading="store.loading"
              @details="(val) => store.pasien = val" @spri="(val) => {
                store.pasien = val
            ...
            <thumbnail-view v-else key="thumbnail-view" :items="store.items" @details="(val) => store.pasien = val"
              @spri="(val) => {
                store.pasien = val
                store.dialogSpri = true
              }" @cetak-gelang="(val) => {
        ...
        <template v-else>
          ...
          <div class="fit column flex-center" v-else>
    ...
    <!-- dialog spri-->
    <dialog-spri v-model="store.dialogSpri" :pasien="store.pasien" :key="store.pasien" />
    <GelangPasienPage v-model="store.dialogCetakGelang" :patien="store.pasien" :key="store.pasien" />
    <!-- <dialog-cetak-gelang
      v-model="store.dialogCetakGelang"
      :pasien="store.pasien"
      :key="store.pasien"
    /> -->
    <dialog-sep v-model="store.dialogSep" :pasien="store.pasien" :key="store.pasien" />
    <dialog-sep-manual v-model="store.dialogSepManual" :pasien="store.pasien" :key="store.pasien" />
    <dialog-cetak-sep v-model="store.dialogCetakSep" :pasien="store.pasien" />
  </div>
</template>
...
import GelangPasienPage from '../../igd/cetak/GelangPasienPage.vue'
