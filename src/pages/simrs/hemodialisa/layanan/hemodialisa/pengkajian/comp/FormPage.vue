  ...
  <div class="row items-center q-col-gutter-x-sm">
    ...
    <div class="col-6">
      <app-input-simrs ref="refAlasan" v-model="store.form.alasan"
        :label="'Alasan Kunjungan ' + panjangChar(store.form.alasan, 250)" :valid="{ max: 255 }" />
    </div>
    <!-- Riwayat Psikososial -->
    <div class="col-6"> <app-input-simrs ref="refRiwayatPsikososial" v-model="store.form.riwayat"
        :label="'Riwayat Psikososial ' + panjangChar(store.form.riwayat, 250)" type="textarea" :valid="{ max: 255 }" />
    </div>
    ...
    <div v-if="store.form.psikologis === 'Lain-lain'" class="col-6">
      <app-input-simrs ref="refLainPsiko" v-model="store.form.lain"
        :label="'Sebutkan Status Psikologis Lainya ' + panjangChar(store.form.lain, 250)" :valid="{ max: 255 }" />
    </div>
    <!-- Tek Dar -->
    <div class="col-5">
      <app-input-simrs ref="refTekDar" v-model="store.form.td"
        :label="'Tekanan Darah ' + panjangChar(store.form.td, 250)" :valid="{ max: 255 }" />
    </div>
    ...
    <div class="col-5">
      <app-input-simrs ref="refTingBad" v-model="store.form.tb"
        :label="'Tinggi Badan ' + panjangChar(store.form.tb, 250)" :valid="{ max: 255 }" />
    </div>
    ...
    <div v-if="store.form.fungsional === 'Perlu Bantuan'" class="col-6">
      <app-input-simrs ref="refLainPsiko" v-model="store.form.lainx"
        :label="'Sebutkan Bantuan yang diperlukan ' + panjangChar(store.form.lainx, 250)" :valid="{ max: 255 }" />
    </div>
  </div>
</template>
<script setup>
