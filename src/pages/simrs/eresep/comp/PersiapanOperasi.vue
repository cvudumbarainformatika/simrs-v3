<template>
  <div class="row full-width bg-primary items-center text-white" style="z-index: 2;">
    <div class="col-auto">
      <HeaderPage :from="store.params.from" :to="store.params.to" :flag="store.params.flag"
        @set-from="store.setParams('from', $event)" @set-to="store.setParams('to', $event)"
        @set-flag="store.setParams('flag', $event)" />
    </div>
    <div class="col text-right q-mr-sm">
      <q-btn flat unelevated round color="yellow" size="sm" icon="icon-mat-refresh" @click="store.refresh">
  ...
  <q-scroll-area class="full-width" style="height: 85vh;">
    <div v-if="store.loading" class="row full-height full-width" style="min-height: 400px;">
      <app-loading />
    </div>
    <div v-if="!store.loading" class="row full-height full-width q-col-gutter-sm q-mb-xl q-mt-sm">
