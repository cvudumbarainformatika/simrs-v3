<template>
  <div class="contentx">
    ...
    <div class="f-12 text-center text-bold q-my-sm">
      <span>NOMOR : <span>{{ store.isisuratkematian?.planheder[0]?.planpulang?.nosurat ||
    ...
    <div class="full-width q-mt-lg">
      ...
      <div class="row q-pa-sm q-ml-md">
        ...
        <div class="col-grow">
          <div class="column">
            <div> : {{ store.isisuratkematian?.datasimpeg?.nama }}</div>
            <div> : {{ store.isisuratkematian?.datasimpeg?.golongan ?? '' }} {{
              store.isisuratkematian?.datasimpeg?.golongan ? ' / ' : '' }} {{
                store.isisuratkematian?.datasimpeg?.ket_golongan ?? '' }}
            </div>
            <div> : {{ store.isisuratkematian?.datasimpeg?.ket_jabatan ?? '' }}</div>
          </div>
        </div>
      </div>
      ...
      <div class="full-width q-mt-md">
        <div class="" style="text-indent: 20px;">
          Pada Tanggal <span class="text-bold"> {{
            humanDate(store.isisuratkematian?.planheder[0]?.planpulang?.tgl_meninggal) }}
          </span>
          tepatnya pada jam : <span class="text-bold"> {{
            store.isisuratkematian?.planheder[0]?.planpulang?.jam_meninggal
            ?? '-' }} WIB </span> ,
          dikarenakan <span class=""> {{ store.isisuratkematian?.planheder[0]?.planpulang?.alasan_meninggal }} </span> .
        </div>
      </div>
      <div class="full-width">
        <div class="" style="text-indent: 20px;">
      ...
      <div class="row justify-between q-mt-xl">
        ...
        <div class="col-auto">
          <div class="column flex-center">
            <div>Probolinggo, Tgl {{ humanDate(store.isisuratkematian?.planheder[0]?.planpulang?.tgl_meninggal) }}</div>
            <div>Mengetahui</div>
            ...
            <div class="q-mt-md">
              {{ store.isisuratkematian?.datasimpeg?.nama || '................................' }}
            </div>
          </div>
        </div>
      </div>
