              ...
              <div> <q-btn flat round size="xs" class="bg-black" color="white" icon="icon-fa-file-regular">
                  <q-menu dark style="min-width: 150px">
                    <q-list style="min-width: 150px;">
                      <q-item v-if="props?.row.kunci !== '1'" clickable @click="editdata(props?.row)">
                        <q-item-section :auth="user">Edit Data</q-item-section>
                      </q-item>
                      ...
                      <q-item v-if="props?.row.kunci !== '1'" clickable @click="deleteData(props?.row?.nosppup)">
...
function deleteData(nosppup) {
  ...
  //   return
  // }
  console.log('nosppup delete', nosppup)
  $q.dialog({
    title: 'Peringatan',
...
function editdata(row) {
  ...
  router.push({ path: '/siasik/panjar/up/formdata', replace: true, query: { id: row.id } })
  editDataref.value = row
  store.form = editDataref.value
