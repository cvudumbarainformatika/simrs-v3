        ...
        <q-list>
          <template v-if="store.datanpd.length !== 0">
            <q-table class="my-sticky-header" wrap-cells :rows="store.datanpd" :columns="columns" row-key="name"
              ...
              <template #top-right>
                <div>
                  <q-badge class="q-pa-sm bg-white text-dark text-bold">
                    Total {{ formatDenganRp(subtotal()) }}
                  </q-badge>
                </div>
              ...
              <template #body="props">
                ...
                <q-tr :props="props">
                  ...
                  <q-td key="ptk" :props="props">
                    <div class="text-bold">
                      {{ props.row.pptk }}
                    </div>
                    <div class="q-pl-sm" style="font-size: 0.9em">{{ props.row.bidang }}</div>
                  </q-td>
                  ...
                  <q-td key="total" :props="props">
                    Rp. {{ formattanpaRp(props.row.total) }}
                  </q-td>
                  <q-td>
...
function subtotal() {
  const subtotalrinci = store.datanpd.map((x) => parseFloat(x.total)).reduce((a, b) => a + b, 0)
