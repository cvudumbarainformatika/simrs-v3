      ...
      <q-card-section class="row items-center">
        <div class="column">
          <div class="f-14 text-weight-bold">
            Nota Dinas {{ store.listrinci[0].nonotadinas }}
          </div>
          <div class="f-12">
        ...
        <q-space />
        <q-btn class="q-mr-sm" color="primary" label="Verifikasi Semua" :disable="selectedNpd.length === 0 || loading"
...
async function verifikasiSemua() {
  if (selectedNpd.value.length === 0) {

    $q.notify({
      type: 'negative',
      message: 'Pilih minimal satu NPD untuk diverifikasi.'
    })
    return
  }
  loading.value = true
  try {
    const notadinas = store.listrinci[0]?.nonotadinas

    const resp = await api.post('/v1/transaksi/notadinas/verifikasi', {
      notadinas,
      datanpd: selectedNpd.value
    })
