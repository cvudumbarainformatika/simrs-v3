  ...
  <div>
    ...
    <q-list v-else bordered class="rounded-borders bg-white" separator square>
      ...
      <q-item v-for="(row, i) in listmaster" :key="row?.id" v-ripple class="q-py-xs">
        ...
        <q-item-section style="max-width: 20%;">{{ row.nip }}</q-item-section>
        <q-item-section style="max-width: 30%;">{{ row.nama }}</q-item-section>
        <q-item-section>{{ row.bagian }}</q-item-section>
        <q-item-section side>
