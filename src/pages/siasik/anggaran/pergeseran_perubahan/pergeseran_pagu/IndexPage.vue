      ...
      <q-card-section class="col-auto">
        <div class="full-width row justify-between items-center content-between">
          ...
          <div class="flex q-gutter-sm">
            ...
            <!-- <q-input v-model="store.params.q" placeholder="Cari Data ..." dense outlined style="min-width:200px"
              debounce="800" :loading="store.loading" @update:model-value="store.search" standout="bg-yellow-3">
              <template #prepend>
                <q-icon name="icon-mat-search" />
              </template>
            </q-input> -->
            <q-input v-model="store.params.q" outlined dense placeholder="Cari Data ..." debounce="500"
              style="min-width: 300px;" @update:model-value="store.search">
...
import { usePergeseranPerubahanPaguStore } from 'src/stores/siasik/anggaran/perubahan_pergeseran/geserperubahan_pagu.js';
const store = usePergeseranPerubahanPaguStore()
const $q = useQuasar()
const options = ref([])
const tahuns = ref([])
...
const totalPagu = computed(() => {
  const items = store.items?.reduce((a, b) => a + Number(b?.pagu_pergeseran), 0)
  return items
})
