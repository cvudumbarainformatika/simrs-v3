...
import { usePergeseranPerubahanPendapatanStore } from 'src/stores/siasik/anggaran/perubahan_pergeseran/perubahanpendapatan.js';
...
const totalPagu = computed(() => {
  const items = store.items?.reduce((a, b) => a + Number(b?.nilai_pergeseran), 0)
  return items
})
function init() {
  const d = new Date()
